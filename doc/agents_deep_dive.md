...
3. **3-Tier Hierarchical Narrative Context**:
   - **Macro Tier**: Series-level premise (`whole_story_summary`) providing foundational character arcs and world conflicts.
   - **Meso Tier**: Active story arc details (`active_arc`: title, milestones, stakes).
   - **Micro Tier**: Rolling synopses of the past 3 chapters (`rolling_summaries[-3:]`), including seamless backfilling across volume folders with badges (e.g. `[Villainess_04] Chapter 122`).
4. **Nickname & Address Form Discipline**:
   - In narrative exposition, characters are strictly anchored to canonical target names (`name_address_fidelity`).
   - In spoken dialogue, informal nicknames (e.g. "Clary", "Little Bao") are only permitted if explicitly voiced in the source text, preventing unauthorized casual drift.
...
- **Name Discipline Steering**: Active node `Name_Discipline` enforces that formal third-person narrative never substitutes colloquial nicknames unless explicitly spoken in dialogue.
